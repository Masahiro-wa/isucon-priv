...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT p.id, p.user_id, p.body, p.created_at, p.mime, u.account_name FROM posts AS p FORCE INDEX(`posts_user_idx`) JOIN `users` AS u ON (p.user_id = u.id) WHERE u.del_flg = 0 ORDER BY p.created_at DESC LIMIT 20"
